...
async function fetchSecureConversationStreams(isInitialLoad = false) {
    ...
    try {
        const r = await fetch(`http://localhost:5000/api/bank/admin-chat?uuid=${activeChatSessionUserUuid}&page=1&limit=${chatMaxLimitPerPage}`, {
            method: "GET",
...
async function fetchOlderHistoricalChatLogs() {
    ...
    try {
        const response = await fetch(`http://localhost:5000/api/bank/admin-chat?uuid=${activeChatSessionUserUuid}&page=${nextPage}&limit=${chatMaxLimitPerPage}`, {
            method: "GET",
            headers: { "Authorization": `Bearer ${adminToken}` }
        });
...
async function dispatchMessagePayload(text, fileUrl, replacementTargetTempId = null) {
    ...
    try {
        const response = await fetch("http://localhost:5000/api/bank/admin-chat", {
            method: "POST",
            headers: {
...
async function clearFileAssetStorageUpload(file) {
    ...
    try {
        const response = await fetch("http://localhost:5000/api/bank/avatar", {
            method: "POST",
            headers: {
                "Authorization": `Bearer ${adminToken}`,
