...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function fetchAssignedSecurityDefaults() {
        ...
        try {
            const connection = await fetch("https://bank-api-v2-peach.vercel.app/api/bank/settings", {
                method: "GET",
                headers: {
    ...
    if (updatedPasswordFormNode) {
        updatedPasswordFormNode.addEventListener("submit", async (e) => {
            ...
            try {
                const response = await fetch("https://bank-api-v2-peach.vercel.app/api/bank/settings", {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        "Authorization": `Bearer ${sessionTokenSignature}`,
